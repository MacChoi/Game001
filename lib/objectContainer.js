...
class File{
    ...
    static onCount(){
        if(--File.fileCount==0)this.removeLoading();
       // console.log("onCount :" +File.fileCount);
    }
    ...
    loadImages(imagePath,imageCount){
        ...
        IMAGES[0] = new Image();
        File.fileCount+=imageCount;
        File.appendLoading();
        for(var i = 1; i<imageCount+1; i++){
            count++;
            IMAGES[i] = new Image();
            IMAGES[i].onload = File.onCount();
